...
namespace FragParty
{
	...
	public class FragPartyController : MonoBehaviour
	{
		...
		private float _diveTimeoutDelta;
		private float _tossTimeoutDelta;
		...
		private void Start()
		{
			...
			_diveTimeoutDelta = diveTimeout;
			_tossTimeoutDelta = tossTimeout;
		...
		private void Slide()
		{
			...
			else if (_animator.GetBool(_animIDSlide) && _slideTime > 0f)
			{
				...
				// a reference to the players current horizontal velocity
				float currentHorizontalSpeed = new Vector3(0.0f, 0.0f, sprintSpeed).magnitude;
